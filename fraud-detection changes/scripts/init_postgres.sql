...
-- =============================================================================
-- TRAINING SCHEMA — Ground truth label store (Milestone B + Production)
-- Written by: feedback-service (real-time) and chargeback_ingestion DAG (batch)
-- =============================================================================

CREATE SCHEMA IF NOT EXISTS training;

-- Ground truth labels for model training
CREATE TABLE IF NOT EXISTS training.labels (
    id               BIGSERIAL    PRIMARY KEY,
    txn_id           VARCHAR(64)  NOT NULL,
    customer_id      VARCHAR(64)  NOT NULL,
    true_label       BOOLEAN      NOT NULL,     -- TRUE = fraud, FALSE = legitimate
    label_source     VARCHAR(32)  NOT NULL       -- CHARGEBACK | ANALYST_FRAUD | ANALYST_LEGIT | SYNTHETIC
                         CHECK (label_source IN ('CHARGEBACK','ANALYST_FRAUD','ANALYST_LEGIT','SYNTHETIC')),
    confidence       DOUBLE PRECISION NOT NULL DEFAULT 1.0 CHECK (confidence BETWEEN 0 AND 1),
    labeled_at       TIMESTAMPTZ  NOT NULL DEFAULT NOW(),
    model_action     VARCHAR(32)  NOT NULL DEFAULT '',
    model_p_fraud    DOUBLE PRECISION DEFAULT 0,
    model_version    VARCHAR(64)  DEFAULT '',
...
CREATE INDEX IF NOT EXISTS idx_training_labels_source     ON training.labels(label_source);
CREATE INDEX IF NOT EXISTS idx_training_labels_labeled_at ON training.labels(labeled_at DESC);
CREATE INDEX IF NOT EXISTS idx_training_labels_true_label ON training.labels(true_label);
CREATE INDEX IF NOT EXISTS idx_training_labels_model_ver  ON training.labels(model_version);

-- Aggregated label stats per model version (maintained by feedback-service)
CREATE TABLE IF NOT EXISTS training.model_performance_snapshots (
    id             BIGSERIAL   PRIMARY KEY,
    model_version  VARCHAR(64) NOT NULL,
    snapshot_at    TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    n_labels       INTEGER     NOT NULL DEFAULT 0,
    n_fraud        INTEGER     NOT NULL DEFAULT 0,
    n_legit        INTEGER     NOT NULL DEFAULT 0,
    ...
    f1_val         DOUBLE PRECISION DEFAULT 0,
    fnr            DOUBLE PRECISION DEFAULT 0,
    fpr            DOUBLE PRECISION DEFAULT 0
);

CREATE INDEX IF NOT EXISTS idx_perf_snapshots_model   ON training.model_performance_snapshots(model_version);
CREATE INDEX IF NOT EXISTS idx_perf_snapshots_snap_at ON training.model_performance_snapshots(snapshot_at DESC);

-- Feature drift snapshots (written by model_monitoring DAG)
CREATE TABLE IF NOT EXISTS training.drift_snapshots (
    id              BIGSERIAL   PRIMARY KEY,
    snapshot_at     TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    overall_psi     DOUBLE PRECISION DEFAULT 0,
    max_psi         DOUBLE PRECISION DEFAULT 0,
    n_drifted       INTEGER     DEFAULT 0,
    drifted_features JSONB      DEFAULT '[]',
    feature_psi     JSONB       DEFAULT '{}',
    retraining_recommended BOOLEAN DEFAULT FALSE
);

CREATE INDEX IF NOT EXISTS idx_drift_snapshots_at ON training.drift_snapshots(snapshot_at DESC);

COMMENT ON TABLE training.labels IS
    'Ground truth fraud labels. Source priority: CHARGEBACK > ANALYST_FRAUD > ANALYST_LEGIT > SYNTHETIC.
     Written by feedback-service (real-time, <30s lag) and chargeback_ingestion DAG (batch, 30min).
     UNIQUE on txn_id — higher-priority label wins via ON CONFLICT UPDATE.';

COMMENT ON TABLE training.model_performance_snapshots IS
    'Rolling FNR/FPR snapshots per model version. Written by feedback-service every 100 labels.';

COMMENT ON TABLE training.drift_snapshots IS
    'Feature distribution drift (PSI) snapshots. Written by model_monitoring Airflow DAG every 6h.';
