...
import { getApiBase, friendlyApiError } from '@/lib/api-health';
...
export function clearTokens() {
  ...
  document.cookie = 'dip_session=; path=/; max-age=0';
}
...
async function apiFetch<T>(
  ...
  if (!res.ok) {
    ...
    if (res.status === 500 && String(message).toLowerCase().includes('internal')) {
      throw new Error('Platform services are starting up. Please try again in a few seconds.');
    }
...
export function connectDecisionStream(onEvent: (event: any) => void): () => void {
  const { access } = getTokens();
  const url = `${getApiBase()}/decisions/stream`;
