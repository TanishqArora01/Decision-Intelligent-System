...
try:
    ...
    PRED_TOTAL   = Counter("stage2_predictions", "Total Stage 2 predictions")
    PRED_LATENCY = Histogram("stage2_latency_ms", "Stage 2 latency (ms)",
                             buckets=[5,10,20,50,100,200,500])
    NEO4J_CALLS  = Counter("stage2_neo4j_calls", "Neo4j graph query calls")
    ANOMALY_HIGH = Counter("stage2_anomaly_high", "Anomaly score > 0.6 count")
except Exception:
    class _NoOp:
