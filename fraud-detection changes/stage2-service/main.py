...
from ensemble.trainer import XGBoostModel, MLPModel
...
try:
    from predictor import PRED_TOTAL, PRED_LATENCY
except Exception:
    class _NoOp:
        def inc(self, v=1): pass
        def observe(self, v): pass
        def labels(self, **k): return self
    PRED_TOTAL = PRED_LATENCY = _NoOp()
...
class Stage2Engine:
    ...
    def score(self, req: Stage2Request) -> Stage2Response:
        ...
        refined_p, confidence = fuse_ensemble(
            xgb_p, mlp_p, config.ensemble_xgb_weight, config.ensemble_mlp_weight
        )
