...
class AnomalyResult(BaseModel):
    ...
    @property
    def combined_anomaly_score(self) -> float:
...
class Stage2Response(BaseModel):
    ...
    customer_id:          str
    amount:               float
    refined_p_fraud:      float = Field(ge=0.0, le=1.0)
    xgb_p_fraud:          float = 0.0
    mlp_p_fraud:          float = 0.0
    ensemble_confidence:  float = Field(ge=0.0, le=1.0, default=0.5)
    graph:                GraphRiskResult = Field(default_factory=GraphRiskResult)
