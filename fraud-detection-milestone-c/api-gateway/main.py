...
cb_stage1 = CircuitBreaker()
cb_stage2 = CircuitBreaker()
cb_stage3 = CircuitBreaker()
...
# Prometheus metrics
# ---------------------------------------------------------------------------
TXN_TOTAL     = Counter("gateway_txn_total",    "Total transactions received")
TXN_EARLY_EXIT= Counter("gateway_early_exit_total", "Early exit approvals")
TXN_ERRORS    = Counter("gateway_errors_total", "Gateway errors", ["stage"])
...
class TransactionRequest(BaseModel):
    ...
    txn_id:          str
    org_id:          str   = "default-org" # Multi-tenant isolation
    customer_id:     str
...
async def run_pipeline(req: TransactionRequest) -> DecisionResponse:
    ...
    if cb_stage1.can_execute():
        try:
            ...
            r1.raise_for_status()
            s1 = r1.json()
            cb_stage1.record_success()
        except Exception as e:
            cb_stage1.record_failure()
            TXN_ERRORS.labels(stage="stage1").inc()
            logger.warning("Stage 1 failed for %s: %s — using fallback", req.txn_id, e)
    else:
        TXN_ERRORS.labels(stage="stage1_cb_open").inc()
        logger.warning("Stage 1 Circuit Breaker OPEN for %s", req.txn_id)
    ...
    if cb_stage2.can_execute():
        try:
            ...
            r2.raise_for_status()
            s2 = r2.json()
            cb_stage2.record_success()
        except Exception as e:
            cb_stage2.record_failure()
            TXN_ERRORS.labels(stage="stage2").inc()
            logger.warning("Stage 2 failed for %s: %s — using Stage 1 score", req.txn_id, e)
    else:
        TXN_ERRORS.labels(stage="stage2_cb_open").inc()
        logger.warning("Stage 2 Circuit Breaker OPEN for %s", req.txn_id)
    ...
    if cb_stage3.can_execute():
        try:
            ...
            r3.raise_for_status()
            s3 = r3.json()
            cb_stage3.record_success()
        except Exception as e:
            cb_stage3.record_failure()
            TXN_ERRORS.labels(stage="stage3").inc()
            logger.warning("Stage 3 failed for %s: %s — using safe default MANUAL_REVIEW", req.txn_id, e)
    else:
        TXN_ERRORS.labels(stage="stage3_cb_open").inc()
        logger.warning("Stage 3 Circuit Breaker OPEN for %s", req.txn_id)
    ...
    final_p_fraud = s2.get("p_fraud", p_fraud)
    explanation = s3.get("explanation", {})

    # ---- Deterministic Rule Engine (Safety Fallbacks) ----
    if final_p_fraud >= 0.95:
        action = "BLOCK"
        action_reason = "[SAFETY_RULE] P(fraud) >= 0.95 strictly blocks."
        explanation["safety_override"] = "Critical risk threshold met."
    elif req.amount > 50000 and action == "APPROVE":
        action = "MANUAL_REVIEW"
        action_reason = "[SAFETY_RULE] High value transaction (>$50k) cannot be auto-approved."
        explanation["safety_override"] = "Manual verification required for high value."
    elif req.is_new_device and req.is_new_ip and final_p_fraud > 0.6 and action == "APPROVE":
        action = "STEP_UP_AUTH"
        action_reason = "[SAFETY_RULE] New device + new IP + moderate risk requires step-up."
        explanation["safety_override"] = "Unrecognized identity requires MFA."

    await _publish_to_kafka(req, action, final_p_fraud, "full_pipeline")
...
async def _publish_to_kafka(req: TransactionRequest, action: str,
    ...
    """Reliable Kafka publish with DLQ fallback."""
    ...
    # DLQ Fallback
    try:
        with open("/tmp/dlq_decisions.log", "a") as f:
