...
async def list_cases(
    ...
    if current_user.org_id:
        conditions.append(ReviewCase.org_id == current_user.org_id)
...
async def sync_from_decisions(
    ...
    try:
        result = await db.execute(text("""
            SELECT txn_id, org_id, customer_id, amount, currency, p_fraud, uncertainty,
    ...
    for r in rows:
        try:
            ...
            case = ReviewCase(
                txn_id           = r["txn_id"],
                org_id           = r.get("org_id", "default-org"),
