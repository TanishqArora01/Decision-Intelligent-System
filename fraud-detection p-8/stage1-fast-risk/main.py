...
class KafkaConsumerThread(threading.Thread):
    ...
    def __init__(self, predictor_provider, stop_event: threading.Event):
...
async def lifespan(app: FastAPI):
    ...
    kafka_thread = KafkaConsumerThread(lambda: app.state.predictor, stop_event)
    kafka_thread.start()
...
@app.post("/reload-models", summary="Reload Stage 1 model and conformal calibrator")
async def reload_models():
    logger.info("Reloading Stage 1 predictor from MLflow / training pipeline...")
    app.state.predictor = build_predictor()
    return {
        "reloaded": True,
        "model_version": app.state.predictor.artifact.model_version,
    }
