...
STAGE1_RELOAD_URL = os.getenv("STAGE1_RELOAD_URL", "http://stage1-service:8100/reload-models")
STAGE2_RELOAD_URL = os.getenv("STAGE2_RELOAD_URL", "http://stage2-service:8200/reload-models")
...
def retrain_stage1(**context):
    ...
    try:
        ...
        stage1_path = "/opt/airflow/stage1-fast-risk"
...
def retrain_stage2_xgb(**context):
    ...
    try:
        ...
        stage2_path = "/opt/airflow/stage2-deep-intelligence"
        if os.path.exists(stage2_path):
            sys.path.insert(0, stage2_path)
            from ensemble.xgboost_model import XGBoostTrainer
            sys.path.insert(0, "/opt/airflow/stage1-fast-risk")
...
def deploy_promoted_models(**context):
    """Reload the live services after MLflow promotion completes."""
    import json
    ...
    if not promoted:
        print("No promoted models — skipping deployment reload")
        return {"reloaded": []}

    targets = []
    if any(name.startswith("stage1_") for name in promoted):
        targets.append(STAGE1_RELOAD_URL)
    if any(name.startswith("stage2_") for name in promoted):
        targets.append(STAGE2_RELOAD_URL)
    ...
    for url in targets:
        try:
            ...
            results.append({"url": url, "status": "ok", "response": body})
            print(f"Reloaded model service: {url}")
        except (HTTPError, URLError, TimeoutError, Exception) as e:
            results.append({"url": url, "status": "error", "error": str(e)})
            print(f"Reload failed for {url}: {e}")

    context["task_instance"].xcom_push(key="deployment_results", value=json.dumps(results))
...
with DAG(
    ...
    deploy = PythonOperator(
        task_id         = "deploy_promoted_models",
        python_callable = deploy_promoted_models,
        trigger_rule    = "all_done",
    )

    # Pipeline topology:
    # fetch → validate → [skip | train_s1 + train_xgb + train_mlp] → evaluate → audit → deploy
    fetch_data >> validate
    validate   >> skip
    validate   >> [train_s1, train_xgb, train_mlp]
    [train_s1, train_xgb, train_mlp] >> evaluate >> audit >> deploy
